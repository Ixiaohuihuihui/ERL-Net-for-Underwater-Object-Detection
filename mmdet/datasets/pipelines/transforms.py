...
class MotionBlur(object):
    ...
    def motion_blur(self, img, img_shape):
        ...
        if img_shape[1] <= 750:
            kernel_size = int(randoms.sample(range(10, 20), 1)[0])
        elif img_shape[1] < 2000 and img_shape[1] > 750:
            kernel_size = int(randoms.sample(range(20, 60), 1)[0])
        elif img_shape[1] >= 2000:
            kernel_size = int(randoms.sample(range(100, 140), 1)[0])
...
class GaussianBlurConv():
            ...
            for j in range(img.shape[0]):
                ...
                for k in range(kernel_size):
                    ...
                    if source > temp.shape[0]:
                        source = math.floor(2 * (img.shape[0] - 1) - source)
    ...
    def FastFilter(self, img, sigma):
        ...
        if kernel_size < 10:
            ...
            sub_img = cv2.pyrDown(img, sub_img)
