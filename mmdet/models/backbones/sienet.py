...
class Bottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        # self.ga = ContextBlock(planes * 4, ratio=1./16, pooling_type='att')
...
class RFB(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, stride=1, scale=0.1, visual=1):
        ...
        self.branch0 = nn.Sequential(
            ...
            BasicConv2d(2 * inter_planes, 2 * inter_planes, kernel_size=3, stride=1, padding=visual, dilation=visual)
        )
        self.branch1 = nn.Sequential(
            ...
            BasicConv2d(2 * inter_planes, 2 * inter_planes, kernel_size=3, stride=1, padding=visual + 1,
                      dilation=visual + 1)
        )
        self.branch2 = nn.Sequential(
            BasicConv2d(in_planes, inter_planes, kernel_size=1, stride=1),
            ...
            BasicConv2d(2 * inter_planes, 2 * inter_planes, kernel_size=3, stride=1, padding=2 * visual + 1,
                      dilation=2 * visual + 1)
        )

        self.ConvLinear = BasicConv2d(6 * inter_planes, out_planes, kernel_size=1, stride=1)
        self.shortcut = BasicConv2d(in_planes, out_planes, kernel_size=1, stride=stride)
...
class SiEdgenet(nn.Module):
    ...
    def __init__(self,
        ...
        self.rf_low_sm = RFB(320, channel)
        self.rf2_sm = RFB(3584, 512)
        self.rf3_sm = RFB(3072, 1024)
        # self.rf_low_sm = RF(576, channel)
        # self.rf2_sm = RF(1792, 512)
        # self.rf3_sm = RF(3072, 1024)
        # self.rf4_sm = RF(2048, 2048)
        # self.rf2_sm = RF(3584, channel)
        # self.rf3_sm = RF(3072, channel)
        # self.rf4_sm = RF(2112, 2048)
        ...
        # self.edge2 = edge_attention(64, 512)
        # self.edge3 = edge_attention(64, 1024)
    ...
    def forward(self, x):
        ...
        # x4_sm_cat = torch.cat((x4_sm, self.downSample(self.downSample(self.downSample(edge)))), dim=1)
        # x4_sm_rf = self.rf4_sm(x4_sm_cat)
